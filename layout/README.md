...
`measurements/2026-09-resistor-fold/README.md` section 5, which compares
against the *pre-fold GDS re-run through the same installed `klt`* rather
than against the committed reports, because the three `sg13cmos5l-*` cells'
committed reports predate a deck build that now recognises `rppd`/`rhigh` --
a deck drift that is visible in this branch's refreshed reports and is not a
