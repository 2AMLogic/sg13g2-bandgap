...

{
  ...
  echo "  drawn W/L/AS/AD/PS/PD) instead of the current schematic's, engages"
  echo "  at cold start and disengages once the core is running, across the"
  echo "  full PVT grid -- **AS DRAWN**, which is w=2u for XMSENSE, not the"
  echo "  schematic's current w=10u (decision record 0003 widened it after"
  echo "  this layout was drawn; the layout has not been regenerated to"
  echo "  match -- see this directory's README.md and the filed follow-up"
  echo "  issue). This direct sns1-sweep alone reads PASS at every point"
  echo "  below (it always completes release by sns1=vdd), but a"
  echo "  cross-bench comparison against core-open-loop-bias-pex's own real"
  echo "  sns1 operating point reproduces decision record 0003's exact same"
  echo "  4 flagged points (wcs_125c_*, sf_125c_3.63v) -- see this"
  echo "  directory's README.md 'Cross-bench observation' for the numbers;"
  echo "  do not read the 45/45 below as clearing that margin bug. Post-"
  echo "  layout (PEX) evidence for issue #14, NOT a claim against any"
  echo "  ratified spec row (none is ratified yet -- see #13)."
  ...
  echo "  \`${DUT_GIT_SHA}\`); the sg13g2 extraction deck does not recognise"
  echo "  resistor devices."
