...

{
  ...
  echo "  (as of issue #176/#158, the extraction now reports a REAL"
  echo "  per-terminal hub leg on \`VSS\` for each body terminal, not the"
  echo "  deck's synthesized \`vsubs\` fallback an earlier revision of this"
  echo "  record described -- this testbench still ties body directly to"
  echo "  the ideal \`vss\` net rather than modelling that hub leg, a"
  echo "  documented simplification, not an unverifiable fixture anymore)."
  echo "  XRPU (rhigh) is now instantiated from the extracted R\$3 device"
  echo "  (issue #59) instead of spliced from the schematic -- real drawn"
  echo "  geometry via an X-subckt call to the real rhigh PDK subckt. As of"
  echo "  issue #176/#158, its \"a\" terminal wires to the real \`vdd__t0\`"
  echo "  hub (the isolated \$5 net an earlier revision of this record"
  echo "  described no longer exists -- this layout now declares a real"
  echo "  \`vdd\` pin), and its bulk terminal ties directly to \`vss\`"
  echo "  (dropped its own real hub leg, same simplification as the nfet"
  echo "  bodies above). See README.md for the current, more nuanced"
  echo "  picture."
