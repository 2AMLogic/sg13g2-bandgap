...
# sim/core-open-loop-bias -- see that experiment's README for the rationale;
# unchanged here since the bipolar/resistor devices are the same
# schematic-sourced instances and the MOS corner axis applies to the same
# sg13_hv_pmos compact model, just with extracted-layout geometry.
...

{
  ...
  echo "  wire parasitics are zero due to a sg13g2 deck gap; PMOS body is a"
  ...
  echo "- **Devices**: XM1/XM2/XM3 -- sg13_hv_pmos, geometry from"
  echo "  \`layout/bandgap_core/bandgap_core.pex.spice\`"
  echo "  (\`klt extract --deck sg13g2 --parasitics\`, layout git sha"
  echo "  \`${LAYOUT_GIT_SHA}\`). XQ1-XQ3 (npn13G2) and XR1/XR2 (rppd) --"
  echo "  spliced verbatim from \`design/netlist/bandgap_core.spice\`"
  echo "  (schematic git sha \`${DUT_GIT_SHA}\`); the sg13g2 extraction deck"
  echo "  does not recognise bipolar or resistor devices (klayout-tools,"
  echo "  filed generically -- see layout/README.md)."
