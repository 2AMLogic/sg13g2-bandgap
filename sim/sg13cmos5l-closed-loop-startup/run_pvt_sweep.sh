...
MSENSE_LINE="$(grep -E '^XMSENSE ' "${REPO_ROOT}/design/sg13cmos5l/netlist/bandgap_startup.spice")"
MSENSE_W="$(echo "${MSENSE_LINE}" | grep -oE 'w=[0-9.]+u' | head -1 | sed -e 's/w=//')"
if [[ -z "${MSENSE_W}" ]]; then
  echo "run_pvt_sweep.sh: could not parse XMSENSE's w= from design/sg13cmos5l/netlist/bandgap_startup.spice" >&2
  exit 3
fi
...
DUT_AMP_GIT_SHA="$(git -C "${REPO_ROOT}" log -1 --format=%h -- design/sg13cmos5l/netlist/bandgap_amp.spice 2>/dev/null || echo unknown)"
